...
use super::{
    LoginRequest, LoginResponse, RefreshRequest, RegisterRequest, Role, TenantInfo, UserInfo,
};
use super::{
    RefreshToken, Tenant, TenantMember, User,
    jwt::JwtManager,
    password::{hash_password, validate_password_strength, verify_password},
};
use crate::audit::{AuditEvent, AuditLogger, actions};
use crate::constants::DEFAULT_TENANT_ID;
...
/// POST /auth/register - Register new user and create default tenant
async fn register(
    ...
    let tenant_slug = generate_unique_slug(&state.storage, &req.email).await?;
    ...
    let (access_token, refresh_token_str) =
        generate_tokens(&state, &user_id, &tenant_id, Role::Owner, None).await?;
...
async fn login(
    ...
    let (tenant, role) = tenants
        .first()
        .ok_or_else(|| BeemFlowError::OAuth("No tenant found".into()))?;

    // 5. Update last login
    state.storage.update_user_last_login(&user.id).await?;

    // 6. Generate tokens (client info will be captured by audit middleware later)
    let (access_token, refresh_token_str) = generate_tokens(
        &state, &user.id, &tenant.id, *role, None, // Client info captured by audit middleware
    )
    .await?;
...
async fn refresh(
    ...
    // 6. Get tenant membership
    let member = state
        .storage
        .get_tenant_member(&refresh_token.tenant_id, &refresh_token.user_id)
        .await?
        .ok_or_else(|| BeemFlowError::OAuth("Membership not found".into()))?;
...
async fn generate_unique_slug(storage: &Arc<dyn Storage>, email: &str) -> Result<String> {
    ...
    // Try base slug first
    if storage.get_tenant_by_slug(&base_slug).await?.is_none() {
        return Ok(base_slug);
    }
    ...
    for _ in 0..10 {
        ...
        if storage.get_tenant_by_slug(&slug).await?.is_none() {
            return Ok(slug);
        }
    }
