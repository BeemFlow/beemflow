...
steps:
  ...
  - id: wait_for_slack_approval
    await_event:
      ...
      timeout: 2h
  - id: handle_edits
    if: "{% if (event.text | lower | trim) is startingwith('edit:') %}true{% endif %}"
    use: core.echo
    with:
      text: "Edits requested: {{ event.text }} (flow would branch to editing here)"
  - id: post_to_socials
    if: "{{ event.text | lower | trim == 'approve' }}"
    parallel: true
    steps:
      - id: x_post
        use: mcp://x/post
...
| Validate flow     | `flow validate <name_or_file>` | `POST /flows/validate`  | `beemflow_validate_flow`   |
...
| Resume run        | `flow resume <token>`    | `POST /runs/resume/{token}` | `beemflow_resume_run`  |
