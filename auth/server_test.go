...
func TestIsValidRedirectURI(t *testing.T) {
	...
	tests := []struct {
		...
		{"localhost HTTPS (prod)", "https://localhost:3330/callback", false, prodServer},
		{"localhost HTTPS (dev)", "https://localhost:3330/callback", true, devServer},
		{"invalid localhost HTTP", "http://localhost:3330/callback", false, prodServer},
		{"invalid 127.0.0.1 HTTP", "http://127.0.0.1:3330/callback", false, prodServer},
